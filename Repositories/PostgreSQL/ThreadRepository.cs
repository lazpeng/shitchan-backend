...
namespace shitchan.Repositories.PostgreSQL
{
    public class ThreadRepository : BaseRepository, IThreadRepository
    {
        ...
        public async Task<List<Thread>> GetThreads(string Board)
        {
            ...
            var query = "SELECT Board, Id as ParentPostId, POSTED as TimestampCreated, (select max(POSTED) from POSTS p where p.PARENT = ID OR p.ID = ID) as TimestampUpdated from POSTS where Board = @Board";
